...
class DatabaseHelper {
  ...
  static final table = 'template'; // テーブル名
  static final task_table = 'template_task'; // テーブル名
  static final task_goal_time = 'template_task_goal_time'; // テーブル名

  static final columnId = 'template_id'; // 列1
  static final columnName = 'template_name'; // 列2
  ...
  Future _onCreate(Database db, int version) async {
    ...
    await db.execute('''
          ...
          CREATE TABLE $task_goal_time(
          $columnId int,
          final_time TEXT NOT NULL,
          foreign key ($columnId) references $table($columnId)
          )
          ''');
    // set_time TEXT NOT NULL,追加予定
  ...
  Future<int> insert(Map<String, dynamic> row) async {
    ...
    return await db.insert(task_table, row); //テーブルにマップ型のものを挿入。追加時のrowIDを返り値にする
  }

  Future<int> insert_task_table(Map<String, dynamic> row) async {
    Database db = await instance.database; //DBにアクセスする
    return await db.insert(task_table, row); //テーブルにマップ型のものを挿入。追加時のrowIDを返り値にする
  }
